...
final class Ma7Wt {
    ...
    private static void keyOff(Slot s) {
        s.lpf.setEgMode(1);
    ...
    private static void envelope(Slot s) {
        switch (s.eg) {
        case KEYON -> {
            s.phase = 0;
            s.eg = Eg.ATTACK;
    ...
    private int osc(Slot s) {
        ...
        if (s.amplitude != 0) {
            ...
            switch (s.osc) {
            case PCM8A -> {
                a = chip.memory[s.wave + i0] << 8;
                b = chip.memory[s.wave + i1] << 8;
            }
            case PCM8B -> {
    ...
    private static int pitchEnvelope(Slot s) {
