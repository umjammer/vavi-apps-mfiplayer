...
/**
 * {@link MfiDeviceProvider} implemented for the rohm sound source.
 *
 * @author <a href="mailto:umjammer@gmail.com">Naohide Sano</a> (nsano)
 * @version 0.00 2026-09-19 nsano initial version <br>
 */
public class RohmMfiDeviceProvider extends MfiDeviceProvider {

    private static final Logger logger = getLogger(RohmMfiDeviceProvider.class.getName());
...
logger.log(Level.ERROR, e.getMessage(), e);
    ...
    public MfiDevice.Info[] getDeviceInfo() {
        return new MfiDevice.Info[] {
                RohmMfiSynthesizer.info,
        };
    }

    @Override
    public MfiDevice getDevice(MfiDevice.Info info) {
        if (info == RohmMfiSynthesizer.info) {
            RohmMfiSynthesizer synthesizer = new RohmMfiSynthesizer();
            return synthesizer;
        } else {
